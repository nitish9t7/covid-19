package app.covid2019.UI.fragment.home;
...
import app.covid2019.adapters.DataListRecyclerAdapter;
...
//        Objects.requireNonNull(getActivity()).setTitle("COVID-19 Data");
...
//        }
//        url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/web-data/archived_data/data/cases.csv";
        url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/web-data/data/cases.csv";
...
//                }
                for (int i = 0; i < formattedRows.size(); i++) {
                    ...
                    DataModel allData = new DataModel(indV.get(3), "as of " + indV.get(4), indV.get(7), indV.get(8), indV.get(9), indV.get(2));
                    dataModelList.add(allData);
                    totConfirmedCases = totConfirmedCases + (int)Double.parseDouble(indV.get(7));
                    totalDeaths = totalDeaths + (int)Double.parseDouble(indV.get(8));
                    try {

                            totalRecovered = totalRecovered + (int)Double.parseDouble(indV.get(9));
                ...
                map.entrySet().forEach(e -> {
                            ...
                            for (int i = 0; i < e.getValue().size(); i++) {
                                ccs += (int)Double.parseDouble(e.getValue().get(i).getConfirmedCases());
                                deaths += (int)Double.parseDouble(e.getValue().get(i).getTotalDeaths());
                                try {
                                    recovered += (int)Double.parseDouble(e.getValue().get(i).getCasesRecovered());
                                }catch (Exception f){
                                    recovered += 0;
                                }
                            }
                ...
                HashSet<String> hashSet = new HashSet<String>(countriesName);
                countriesName.clear();
    ...
    private void setUpRecyclerView(List<DataModel> dm) {
        ...
        DataListRecyclerAdapter dataListRecyclerAdapter = new DataListRecyclerAdapter(dm);
